...
function A.GPH_SetRarityProtection(q, value)
    ...
    if A.RefreshBagUIs then
        ...
        local bf = A.Bank
        if bf then bf._refreshImmediate = true end
...
--- Handle Alt-Click item protection toggle.
function A.HandleGearProtection(currentEquipped, lastEquippedItemIds)
    local SV = _G.FugaziBAGSDB
    if not SV then return end
    
    local protected = A.GetGphProtectedSet()
    local ledger = A.GetGphWornLedger()
    local key = A.GetGphCharKey()
    ...
    -- 1. Protect items that were in lastEquipped but are now unequipped
    ...
    -- 2. Clear manual unprotect for anything currently worn (Ensure future re-protect)
...
function A.ToggleItemProtection(itemId, link, clickArea)
    ...
    if clickArea and clickArea.GetParent then
        local row = clickArea:GetParent()
        if row and A.TriggerRowPulse and (not protNow) then A.TriggerRowPulse(row) end
    end
end
