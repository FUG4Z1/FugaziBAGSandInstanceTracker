...
local function GetTabRunIndicesAndOrdinal(history, selectedTab, detailPage)
    ...
    if selectedTab == 2 then
        for i, run in ipairs(history) do
            if IsGPHRun(run) and (not run.realmName or run.realmName == currentRealm) then
                indices[#indices + 1] = i
            end
        end
    elseif selectedTab == 3 then
        for i, run in ipairs(history) do
            if not IsGPHRun(run) and (run.name or "") ~= "" and (not run.realmName or run.realmName == currentRealm) then
                indices[#indices + 1] = i
            end
        end
    else
...
function L.CreateStatsFrame()
    ...
    detailPrevBtn:SetScript("OnClick", function()
        ...
        local res = GetTabRunIndicesAndOrdinal(history, tab, detailPage)
        local indices = res.indices
        local ordinal = res.ordinal
        if ordinal <= 1 then
            f.selectedTab = 1
            if ledgerDetailFrame and ledgerDetailFrame:IsShown() then ledgerDetailFrame:Hide() end
            if type(UpdateStatsTabs) == "function" then UpdateStatsTabs() end
        else
            local prevIdx = indices[ordinal - 1]
    ...
    detailNextBtn:SetScript("OnClick", function()
        ...
        local detailPage = (ledgerDetailFrame and ledgerDetailFrame.detailPage) or 1
        if tab == 1 then
            -- Lifetime: go to the first run of the current realm
            local currentRealm = (GetRealmName and GetRealmName()) or ""
            local firstRunIdx = nil
            ...
            if not firstRunIdx then return end
            local firstRun = history[firstRunIdx]
            ...
            if type(UpdateStatsTabs) == "function" then UpdateStatsTabs() end
            if type(ShowLedgerDetail) == "function" then ShowLedgerDetail(firstRunIdx) end
            ...
            -- Sessions or Dungeons: next run in record order for this tab/realm
            local res = GetTabRunIndicesAndOrdinal(history, tab, detailPage)
            local indices = res.indices
            local ordinal = res.ordinal
            if ordinal < #indices then
                local nextIdx = indices[ordinal + 1]
...
function L.RefreshStatsLifetimeUI(content)
    ...
    -- Lifetime Gold = total gold ever gained (current realm); mouseover shows per-character (current realm)
    local totalGained = 0
    for key, v in pairs(InstanceTrackerDB.lifetimeGoldGained or {}) do
        local r = key:match("^(.-)#")
        if r == currentRealm then
            totalGained = totalGained + (v or 0)
        end
    end
    ...
    -- Deaths = all lifetime deaths (current realm); mouseover per character (current realm)
    local totalDeaths = 0
    for key, v in pairs(InstanceTrackerDB.lifetimeDeaths or {}) do
        local r = key:match("^(.-)#")
        if r == currentRealm then
            totalDeaths = totalDeaths + (v or 0)
        end
    end
    ...
    local vendStats = L.autoVendorStats
...
function L.RefreshStatsSummaryUI(content)
    ...
    for key, v in pairs(AG2) do
        local r = key:match("^(.-)#")
        if r == currentRealm then
            accountGoldTotal2 = accountGoldTotal2 + (v or 0)
        end
    end
    ...
    for key, v in pairs(InstanceTrackerDB.lifetimeGoldGained or {}) do
        local r = key:match("^(.-)#")
        if r == currentRealm then
            totalGained2 = totalGained2 + (v or 0)
        end
    end
    ...
    local delStats2 = L.autoDeleteStats
...
L.RefreshLedgerDetailUI = function(forceRebuild)
    ...
    if statsFrame and statsFrame.detailNavPageLabel then
        local tab = statsFrame.selectedTab or 1
        if tab == 1 then
            local currentRealm = (GetRealmName and GetRealmName()) or ""
            ...
            statsFrame.detailNavPageLabel:SetText("Run 1 of " .. totalRealmRuns)
            statsFrame.detailNavPrevBtn:Hide()
            if totalRealmRuns > 0 then statsFrame.detailNavNextBtn:Show() else statsFrame.detailNavNextBtn:Hide() end
        else
            local res = GetTabRunIndicesAndOrdinal(history, tab, page)
            local nValid = #res.indices
            local ordinal = res.ordinal
            if ordinal == 0 then ordinal = 1 end
            statsFrame.detailNavPageLabel:SetText("Run " .. ordinal .. " of " .. nValid)
            if ordinal > 1 then statsFrame.detailNavPrevBtn:Show() else statsFrame.detailNavPrevBtn:Hide() end
            if ordinal < nValid then statsFrame.detailNavNextBtn:Show() else statsFrame.detailNavNextBtn:Hide() end
        end
    end
end
...
RefreshStatsUI = function(forceRebuild)
        ...
        if frame.detailNavPageLabel then
            if tab == 1 then
                ...
                if totalRealmRuns == 0 then
                    ...
                    frame.detailNavPageLabel:SetText("Run 1 of " .. totalRealmRuns)
                    frame.detailNavPrevBtn:Hide()
                ...
                if nValid == 0 then
                    ...
                    local ordinal = res.ordinal
                    if ordinal == 0 then ordinal = 1 end
                    frame.detailNavPageLabel:SetText("Run " .. ordinal .. " of " .. nValid)
                    if ordinal > 1 then frame.detailNavPrevBtn:Show() else frame.detailNavPrevBtn:Hide() end
                    if ordinal < nValid then frame.detailNavNextBtn:Show() else frame.detailNavNextBtn:Hide() end
                end
            end
        end
    end
    ...
    local currentRealm = (GetRealmName and GetRealmName()) or ""
    for i, run in ipairs(history) do
        if (not run.realmName or run.realmName == currentRealm) and (not tabFilter or tabFilter(run)) then
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "PLAYER_DEAD" then
        ...
        if L.LS and (currentRun or (IsInInstance and IsInInstance())) then
            local LS = L.LS
...
eventFrame:HookScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        -- ============================================================================
        -- FugaziBAGS Modular Bridge Integration
        -- ============================================================================
        -- If __FugaziBAGS is present, we inject our Session and Tracker menus there
        -- to keep the Bags code clean.
        local function InjectBagsMenu()
            local Addon = _G.__FugaziBAGS or _G.FugaziBAGS or _G.FugaziBAGS
            if not Addon then return end

        -- Injection Point 1: Session Start/Stop (Above Notepad)
        ...
        -- Injection Point 2: Tracker/Stats (Below Notepad)
